...
def calcular_estado(precio_almuerzo, almuerzos_diarios, dias, precio_carne,
    ...
    gastos = gasto_en_personal + comisiones_totales + arrendamiento + depreciacion + impuestos
    ...
    return {
        ...
        "arrendamiento": arrendamiento, "depreciaciones": depreciacion,
        "impuesto": impuestos,
...
def analisis_sensibilidad_costos_gastos(base_params):
    ...
    for var_costo in variacion:
        for var_gasto in variacion:
            res = calcular_estado(
                precio_almuerzo, almuerzos, dias,
                precio_carne * var_costo, 75, 0.005,
                arrendamiento=850000 * var_gasto
            )
            resultados.append({
                'var_costo': var_costo,
                'var_gasto': var_gasto,
...
def analisis_sensibilidad_variables(base_params):
    ...
    for var in ['precio', 'almuerzos', 'dias', 'carne']:
        if var == 'precio':
            ...
            for val in valores:
                res = calcular_estado(val, almuerzos_base, dias_base, carne_base, 75, 0.005)
                resultados.append({
            ...
            for val in valores:
                res = calcular_estado(precio_base, val, dias_base, carne_base, 75, 0.005)
                resultados.append({
            ...
            for val in valores:
                res = calcular_estado(precio_base, almuerzos_base, val, carne_base, 75, 0.005)
                resultados.append({
            ...
            for val in valores:
                res = calcular_estado(precio_base, almuerzos_base, dias_base, val, 75, 0.005)
                resultados.append({
        ...
        with tabs[1]:
            ...
            st.markdown("""
                **Objetivo**: Analizar cómo varía la utilidad antes de impuestos cuando cambian:
                - Costo de ventas (principalmente precio de la carne)
                - Gastos de administración y ventas (arrendamiento)
                """)

            if st.button("Ejecutar Análisis 1.1"):
                base_params = {
                    'precio': (3500, 4000),
                    'ventas': (100, 120),
                    'dias': (23, 23),
                ...
                with st.spinner("Analizando..."):
                    ...
                    sns.heatmap(pivot, annot=True, fmt='.0f', cmap='RdYlGn', ax=ax,
                                center=0, cbar_kws={'label': 'Utilidad ($)'})
                    ax.set_xlabel('Variación Gastos (1.0 = base)')
                    ax.set_ylabel('Variación Costos (1.0 = base)')
                    ax.set_title('Sensibilidad: Utilidad vs Costos y Gastos')
                    st.pyplot(fig)

                    st.subheader("Interpretación")
                    st.write("""
                        - **Eje Y (Costos)**: Un aumento del 20% en costos (precio carne) reduce significativamente la utilidad
                        - **Eje X (Gastos)**: Los gastos tienen menor impacto relativo que los costos
                        - **Zona verde**: Escenarios rentables
                        - **Zona roja**: Escenarios de pérdida
                        """)

        # --- TAB 3: Análisis Sensibilidad Variables (2.1) ---
        with tabs[2]:
            st.header("2.1 - Análisis de Sensibilidad: Variables Principales")
            st.markdown("**Objetivo**: Analizar el impacto individual de cada variable en ventas y utilidad")

            if st.button("Ejecutar Análisis 2.1"):
                base_params = {
                    'precio': (3500, 4000),
                    'ventas': (100, 120),
                    'dias': (22, 25),
                ...
                with st.spinner("Analizando variables..."):
                    ...
                    for var in df_vars['variable'].unique():
                        ...
                        ax1.plot(data['valor'], data['ventas'], marker='o', label=var)
                    ax1.set_xlabel('Valor de Variable')
                    ax1.set_ylabel('Ventas ($)')
                    ax1.set_title('Impacto en Ventas')
                    ax1.legend()
                    ...
                    for var in df_vars['variable'].unique():
                        data = df_vars[df_vars['variable'] == var]
                        ax2.plot(data['valor'], data['utilidad'], marker='o', label=var)
                    ax2.set_xlabel('Valor de Variable')
                    ax2.set_ylabel('Utilidad ($)')
                    ax2.set_title('Impacto en Utilidad')
                    ax2.legend()
